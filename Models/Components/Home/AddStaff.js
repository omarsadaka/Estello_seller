...
import {  SetLoading , addStaff} from './../../Actions' //redux
    ...
    UNSAFE_componentWillMount(){
      ...
      componentWillReceiveProps(nextProps) {
         if (nextProps.Message != null) {
             if(nextProps.Message=='Add Staff Done'){
                alert('Add Staff Done')
    ...
    renderHeader() {
        return (
           <View style={[this.props.Language=='AR'? styles.row:styles.row_res, styles.shadow, { width: width, height: '8%', alignItems: 'center', backgroundColor: '#383B43', paddingHorizontal: 18 }]} >
             ...
             <Text style={{ flex:1,textAlign:'center',color: '#fff', fontSize: 17,fontFamily:'nexa_bold',}}>{this.props.Language=='AR'?'أضافة عاملين':'Add Staff'}</Text>
                ...
                style={{flex:1,margin:5 ,height:50 , borderRadius:12,alignItems:'center',justifyContent:'center',backgroundColor:this.state.bg3}}>
                 <Text style={{flex:1,textAlign:'center',textAlignVertical:'center',color:'#383B43',fontSize:15,fontFamily:'nexa_light'}}>
                   Seller</Text>
                        ...
                        <View style={[styles.row, { justifyContent: 'center', alignItems: 'center', marginTop: 15 }]}>
                            <TouchableOpacity 
                            onPress={() => {
                                 const { name , email , phone , password , conf_password , authorization}= this.state
                                    if(name){
                                        if(email){
                                               ...
                                               if(phone){
                                                   if(password){
                                                       if(password==conf_password){
                                                           if(authorization){
                                                            this.props.addStaff(this.props.User.token,name,email,phone,password,authorization)
                                                           }else{
                                                               alert('Enter Authorization First')
                                                           }
                                                       }else{
...
// redux
export default connect(mapStateToProps, { SetLoading , addStaff})(AddStaff)
