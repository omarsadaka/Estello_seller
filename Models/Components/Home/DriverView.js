...
import {  SetLoading , resetPwd , assignDriver} from './../../Actions' //redux
    ...
    renderHeader() {
        return (
           <View style={[this.props.Language=='AR'? styles.row:styles.row_res, styles.shadow, { width: width, height: '8%', alignItems: 'center', backgroundColor: '#383B43', paddingHorizontal: 18 }]} >
             ...
             <Text style={{ flex:1,textAlign:'center',color: '#fff', fontSize: 17,fontFamily:'nexa_bold',}}>{this.props.Language=='AR'?'تفاصيل السائـق':'Driver View'}</Text>
        ...
        return (
            <View style={styles.container} >
                ...
                <View style={{width:width , alignItems:'center',justifyContent:'center'}}>
                 ...
                 <View style={[this.props.Language=='AR'?styles.row_res:styles.row,{width:'80%',alignItems:'center',justifyContent:'center',marginTop:height*0.1}]}>
                 ...
                 {this.state.driverData.name}</Text>
                  ...
                  <View style={[this.props.Language=='AR'?styles.row_res:styles.row,{width:'80%',alignItems:'center',justifyContent:'center',marginTop:10}]}>
                  <Text style={{color:'#383B4370',fontSize:15,fontFamily:'nexa_bold'}}>
                  {this.props.Language=='AR'?'الأيميـل :':'Email :'}</Text>
                 <Text style={[this.props.Language=='AR'?styles.right:styles.left,{ flex:1,color:'#383B4370',paddingHorizontal:7,fontSize:15,fontFamily:'nexa_bold'}]}>
                 {this.state.driverData.email}</Text>
                 </View>
                 ...
                 <Text style={[this.props.Language=='AR'?styles.right:styles.left,{ flex:1,color:'#383B4370',paddingHorizontal:7,fontSize:15,fontFamily:'nexa_bold'}]}>
                 {this.state.driverData.phone}</Text>
                 </View>
                 ...
                 <Text style={[this.props.Language=='AR'?styles.right:styles.left,{ flex:1,color:'#383B4370',paddingHorizontal:7,fontSize:15,fontFamily:'nexa_bold'}]}>
                 {this.state.driverData.status}</Text>
                 </View>
{/*                       
               ...
               <TouchableOpacity 
                ...
                style={{flex:1,margin:5 ,height:50 , borderRadius:12,alignItems:'center',justifyContent:'center',backgroundColor:this.state.bg3}}>
                 ...
                 onPress={()=> this.setState({isVisible: true})}
                  style={[styles.shadow,styles.view,{marginTop:height*0.1}]}>
                ...
                <TouchableOpacity 
                  onPress={()=> this.props.navigation.navigate('DriverProfile')}
                ...
                <TouchableOpacity onPress={() => { 
                 this.props.assignDriver(this.props.User.token , this.state.driverData.id , this.state.driverData.order_id)
                // alert( this.state.driverData.order_id)
                }} 
                style={[styles.Button, styles.shadow, { width: '35%', backgroundColor: '#383B43',paddingVertical:15 }]} >
                  <Text style={{ color: '#FFCF06', fontSize: 20,fontFamily:'nexa_bold', }}>
                       {this.props.Language == "AR" ? 'تخصيص' : 'Assign'}
                  </Text>
                </TouchableOpacity>
              </View>
          ...
          <View style={styles.modal}>
            ...
            <View style={[styles.row, { justifyContent: 'center', alignItems: 'center' }]} >
                ...
                onPress={() => { 
                    ...
                    if(password){
                     this.props.resetPwd(this.props.User.token , password , this.state.driverData.id)
                    }else{
...
// redux
export default connect(mapStateToProps, { SetLoading , resetPwd , assignDriver})(DriverView)
